...
class TSSDWithAttention(nn.Module):  # Res-TSSDNet with Attention
    ...
    def forward(self, x):
        ...
        # Apply attention
        x = x.permute(2, 0, 1)  # Reshape for attention (seq_len, batch, features)
        x, _ = self.attention(x, x, x)  # Self-attention
        x = x.permute(1, 2, 0)  # Reshape back (batch, features, seq_len)

        # Final pooling with the remaining length
        x = F.max_pool1d(x, kernel_size=x.shape[-1])  # This will collapse to 1D
...
if __name__ == "__main__":
    model = TSSDWithAttention(in_dim=64600)
